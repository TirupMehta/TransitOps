...
import { LogOut, ChevronLeft, ChevronRight, ShieldAlert } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`bg-card-theme flex flex-col justify-between shrink-0 h-full border-r border-theme shadow-[4px_0_12px_rgba(0,0,0,0.05)] transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'
        }`}
    >
      <div>
        {/* Brand Header */}
        <div className={`flex border-b border-theme bg-card-theme shrink-0 relative transition-all duration-300 ${isCollapsed ? 'flex-col items-center py-4 gap-3' : 'items-center justify-between p-4 h-16'
          }`}>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative group w-full flex items-center rounded-xl text-sm font-bold transition-all cursor-pointer ${isCollapsed ? 'justify-center p-3' : 'gap-3 px-4 py-2.5 text-left'
                      } ${isActive
      ...
      <div className="p-3 border-t border-theme bg-card-theme space-y-3">
        ...
        <button
          ...
          className={`w-full flex items-center gap-2.5 px-3 py-2 hover:bg-rose-500/5 border border-transparent hover:border-[#eccbc1]/20 hover:text-red-500 rounded-xl text-xs font-extrabold transition-all text-secondary cursor-pointer ${isCollapsed ? 'justify-center' : ''
            }`}
